...
module.exports.set = function (app) {
        ...
        for (var i = 0; i < res.locals.matches.length; i++) {
          ...
          if (
            ...
            await res.locals.client
              ...
              .updateOne(
                { _id: idOfUser },
                { $set: { team: res.locals.matches, game: res.locals.game } }
              );
          } else {
  ...
  async function findUserByPreferences(req, res, next) {
    res.locals.client = new MongoClient(uri, {
    ...
    try {
      ...
      await res.locals.client
        ...
        .toArray((error, documents) => {
          ...
          console.log(preferences);
          const resultNew = res.locals.client
            .db("partyScoutUsers")
            .collection(collection_name)
